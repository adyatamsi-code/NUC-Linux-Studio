...
from .usb_utils import rebind_ite_driver
...
class KeyboardController:
    ...
    def set_effect(self, effect: str, color: str = "random", speed: int = 3, brightness: int = 50, reactive: bool = False, direction: str = None) -> None:
        ...
        if effect == "audio":
            # color here may be a hex string like "#ff0080" or None
            audio_color = None
            if color and isinstance(color, str) and color.startswith("#") and len(color) == 7:
                try:
                    audio_color = [int(color[1:3], 16), int(color[3:5], 16), int(color[5:7], 16)]
                except Exception:
                    pass
            self._start_audio_reactive(brightness, direction=direction, color=audio_color)
            return
        self._stop_audio_reactive()
    ...
    def _stop_audio_reactive(self):
        """Stop audio-reactive mode by clearing the daemon state file and stopping the service."""
        ...
        if self._ite_dev:
            try:
                ...
                pass
        self._ite_dev = None
        # Signal the audio daemon to stop via state file
        try:
            import json
            self._AUDIO_STATE_FILE.write_text(json.dumps({"active": False}))
        except Exception:
            pass
        # Stop the systemd service (on-demand — not auto-started)
        try:
            import subprocess as _sp
            _sp.run(["systemctl", "stop", "kbd-audio.service"],
    ...
    def _rebind_kernel_driver(self):
        """Rebind the ITE8291R3 USB interface to the kernel driver (delegates to usb_utils)."""
        rebind_ite_driver()

    def _start_audio_reactive(self, brightness=100, direction=None, color=None):
        ...
        When switching audio→audio (just changing color/direction/brightness), we write
        the new state file directly without stopping first — the daemon picks up the new
        settings on its next 0.3s state-check cycle. Stop+rebind is only done when the
        daemon is currently idle (not already in audio mode).
        """
        import json

        # Check if daemon is currently active — if so, update in-place (no USB rebind needed)
        currently_active = False
        try:
            if self._AUDIO_STATE_FILE.exists():
                old = json.loads(self._AUDIO_STATE_FILE.read_text())
                currently_active = old.get("active", False)
        except Exception:
            pass

        if not currently_active:
            # Daemon was idle — do a clean stop/rebind cycle first
            self._stop_audio_reactive()
        ...
        state = {
            ...
            "color": color,  # None → rainbow, [R,G,B] → single-color
        }
        try:
            self._AUDIO_STATE_FILE.parent.mkdir(parents=True, exist_ok=True)
            self._AUDIO_STATE_FILE.write_text(json.dumps(state))
            print(f"Audio mode: signalled daemon (brightness={brightness}, direction={direction}, color={color})", flush=True)
        except Exception as e:
            print(f"Audio mode: failed to write state file: {e}", flush=True)

        # Start the service if not already running (on-demand — 0.5s delay for USB release safety)
        if not currently_active:
            import subprocess as _sp, threading as _th
            def _delayed_start():
                time.sleep(0.5)
                try:
                    result = _sp.run(["systemctl", "is-active", "--quiet", "kbd-audio.service"],
                                     capture_output=True, timeout=3)
                    if result.returncode != 0:
                        _sp.run(["systemctl", "start", "kbd-audio.service"],
                                capture_output=True, timeout=10)
                        print("Audio mode: started kbd-audio.service", flush=True)
                except Exception as _e:
                    print(f"Audio mode: failed to start kbd-audio.service: {_e}", flush=True)
            _th.Thread(target=_delayed_start, daemon=True).start()
...
class LightbarController:
    def __init__(self):
        ...
        self._dynamic_rainbow_thread = None
        self._dynamic_rainbow_stop = threading.Event()

    def reset(self) -> None:
        """Reset the lightbar to a clean off state by flushing all EC settings."""
        self._stop_breathing()
        self._stop_dynamic_rainbow()
    ...
    def set_color(self, rgb: Tuple[int, int, int], brightness: int = 100) -> None:
        # Stamp timestamp so kbd_brightness_daemon suppresses the EC echo OSD
        try:
            import time as _t
            Path("/tmp/nuc_lightbar_write_ts").write_text(str(_t.time()))
        except Exception:
            pass
        self._stop_breathing()
        self._stop_dynamic_rainbow()
    ...
    def set_effect(self, effect: str, color: Optional[Tuple[int, int, int]] = None, brightness: int = 100) -> None:
        ...
        if effect == "rainbow":
            ...
            return
        if effect == "dynamic_rainbow":
            # Software-driven scrolling rainbow — writes RGB cycling through hues to sysfs
            writes = {}
            if self.rainbow_path.exists():
                writes[self.rainbow_path] = "0"
            if self.breathing_path and self.breathing_path.exists():
            ...
            if writes:
                write_multiple(writes)
            self._start_dynamic_rainbow(brightness)
            return
    ...
    def _stop_dynamic_rainbow(self):
        """Stop software dynamic rainbow thread if running."""
        if self._dynamic_rainbow_thread and self._dynamic_rainbow_thread.is_alive():
            self._dynamic_rainbow_stop.set()
            self._dynamic_rainbow_thread.join(timeout=2)
        self._dynamic_rainbow_stop.clear()

    def _start_dynamic_rainbow(self, brightness: int = 100):
        """Start a software scrolling rainbow effect using hue cycling."""
        ...
        def _cycle():
            ...
            try:
                ...
                pass
            step_time = 0.033  # ~30fps
            while not self._dynamic_rainbow_stop.is_set():
                r, g, b = _csys.hsv_to_rgb(hue, 1.0, 1.0)
                ri = int(r * 255 * brightness / 100 * max_brightness / 255)
                gi = int(g * 255 * brightness / 100 * max_brightness / 255)
                bi = int(b * 255 * brightness / 100 * max_brightness / 255)
                try:
                    intensity_path.write_text(f"{ri} {gi} {bi}")
                except Exception:
                    return
                hue = (hue + 0.005) % 1.0
                self._dynamic_rainbow_stop.wait(step_time)

        self._dynamic_rainbow_thread = threading.Thread(target=_cycle, daemon=True)
        self._dynamic_rainbow_thread.start()

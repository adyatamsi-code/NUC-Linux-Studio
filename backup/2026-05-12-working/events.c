...
static const struct key_entry nuc_wmi_wmi_hotkeys[] = {
	...
	/* touchpad toggle (reported by WMI and i8042 filter) */
	{ KE_KEY,    0x04, { KEY_F21 }},
	{ KE_KEY,    0x05, { KEY_F21 }},
...
static void process_event_72(const union acpi_object *obj)
{
	...
	/* touchpad off (event 4 = EC disabling touchpad) */
	case 4:
		pr_info("touchpad off\n");
		{
			int status = ec_read_byte(CTRL_4_ADDR);
			if (status >= 0)
				ec_write_byte(CTRL_4_ADDR, status | CTRL_4_TOUCHPAD_TOGGLE_OFF);
		}
		break;

	/* touchpad on (event 5 = EC enabling touchpad) */
	case 5:
		pr_info("touchpad on\n");
		{
			int status = ec_read_byte(CTRL_4_ADDR);
	...
	/* mic mute (alternate code on NUC X15) */
	case 183:
		pr_info("mic mute\n");
		break;
...
{
	/* Touchpad toggle detected via i8042 keyboard sequence.
	 * Log for dmesg-based daemon and report KEY_F21. */

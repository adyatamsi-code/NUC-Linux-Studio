...
class UniwillBackend:
    def __init__(self):
        ...
        try:
            self._fans = FanController(self._find_hwmon_path())
    ...
    @staticmethod
    def _find_hwmon_path():
        from pathlib import Path
        for base in [Path("/sys/devices/platform/nuc_wmi/hwmon"),
                     Path("/sys/devices/platform/qc71_laptop/hwmon")]:
            if base.exists():
                for entry in sorted(base.iterdir()):
                    if entry.is_dir() and entry.name.startswith("hwmon"):
                        return entry
        # Fallback: search /sys/class/hwmon for coretemp or similar
        hwmon_base = Path("/sys/class/hwmon")
        if hwmon_base.exists():
            for entry in sorted(hwmon_base.iterdir()):
                if (entry / "temp1_input").exists():
                    return entry

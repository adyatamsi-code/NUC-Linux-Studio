...
static const struct nuc_wmi_debugfs_attr {
	const char *name;
	uint16_t addr;
} nuc_wmi_debugfs_attrs[] = {
	{"1108", 1108},
...
static struct dentry *nuc_wmi_debugfs_dir,
		     *nuc_wmi_debugfs_regs_dir;

/* ========================================================================== */

static int get_debugfs_byte(void *data, u64 *value)
{
	const struct nuc_wmi_debugfs_attr *attr = data;
...
DEFINE_DEBUGFS_ATTRIBUTE(nuc_wmi_debugfs_fops, get_debugfs_byte, set_debugfs_byte, "0x%02llx\n");

/* ========================================================================== */

static ssize_t nuc_wmi_debugfs_ec_read(struct file *f, char __user *buf, size_t count, loff_t *offset)
{
	size_t i;
...
static ssize_t nuc_wmi_debugfs_ec_write(struct file *f, const char __user *buf, size_t count, loff_t *offset)
{
	size_t i;
...
static const struct file_operations nuc_wmi_debugfs_ec_fops = {
	.owner = THIS_MODULE,
	.open = simple_open,
	.read = nuc_wmi_debugfs_ec_read,
	.write = nuc_wmi_debugfs_ec_write,
...
int __init nuc_wmi_debugfs_setup(void)
{
	...
	nuc_wmi_debugfs_dir = debugfs_create_dir(DEBUGFS_DIR_NAME, NULL);
	if (IS_ERR(nuc_wmi_debugfs_dir)) {
		err = PTR_ERR(nuc_wmi_debugfs_dir);
		goto out_error;
	}

	nuc_wmi_debugfs_regs_dir = debugfs_create_dir("regs", nuc_wmi_debugfs_dir);
	if (IS_ERR(nuc_wmi_debugfs_regs_dir)) {
		err = PTR_ERR(nuc_wmi_debugfs_regs_dir);
		goto out_error_remove_dir;
	}

	for (i = 0; i < ARRAY_SIZE(nuc_wmi_debugfs_attrs); i++) {
		const struct nuc_wmi_debugfs_attr *attr = &nuc_wmi_debugfs_attrs[i];

		d = debugfs_create_file(attr->name, 0600, nuc_wmi_debugfs_regs_dir,
					(void *) attr, &nuc_wmi_debugfs_fops);
		if (IS_ERR(d)) {
			err = PTR_ERR(d);
			goto out_error_remove_dir;
		}
	...
	debugfs_create_file_size("ec", 0600, nuc_wmi_debugfs_dir, NULL, &nuc_wmi_debugfs_ec_fops, U16_MAX);

	return 0;

out_error_remove_dir:
	debugfs_remove_recursive(nuc_wmi_debugfs_dir);
out_error:
	return err;
}

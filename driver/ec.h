...
#ifndef NUC_WMI_LAPTOP_EC_H
#define NUC_WMI_LAPTOP_EC_H
...
#define STA0_ADDR ADDR(0x07, 0x68)
...
#define LIGHTBAR_BRIGHTNESS_ADDR ADDR(0x07, 0x47)
#define LIGHTBAR_CTRL_ADDR       ADDR(0x07, 0x48)
#define LIGHTBAR_CTRL_STATIC     0x01
#define LIGHTBAR_CTRL_BREATH     BIT(5)
...
/* All mutually exclusive mode bits — clear before setting a new mode */
#define LIGHTBAR_CTRL_MODE_MASK  (LIGHTBAR_CTRL_STATIC | LIGHTBAR_CTRL_BREATH | LIGHTBAR_CTRL_RAINBOW)

/* Lightbar color registers — RGB order per qc71_laptop driver */
#define LIGHTBAR_RED_ADDR   ADDR(0x07, 0x49)
#define LIGHTBAR_GREEN_ADDR ADDR(0x07, 0x4A)
#define LIGHTBAR_BLUE_ADDR  ADDR(0x07, 0x4B)

/* Max EC color value — 0-36 range (steps of 4 in OEM firmware) */
#define LIGHTBAR_COLOR_MAX  36
...
int __must_check nuc_wmi_ec_transaction(uint16_t addr, uint16_t data,
				     union nuc_wmi_ec_result *result, bool read);

static inline __must_check int nuc_wmi_ec_read(uint16_t addr, union nuc_wmi_ec_result *result)
{
	return nuc_wmi_ec_transaction(addr, 0, result, true);
}

static inline __must_check int nuc_wmi_ec_write(uint16_t addr, uint16_t data)
{
	return nuc_wmi_ec_transaction(addr, data, NULL, false);
}

static inline __must_check int ec_write_byte(uint16_t addr, uint8_t data)
{
	return nuc_wmi_ec_write(addr, data);
}

static inline __must_check int ec_read_byte(uint16_t addr)
{
	union nuc_wmi_ec_result result;
	int err;

	err = nuc_wmi_ec_read(addr, &result);

...
static const struct dmi_system_id nuc_wmi_dmi_table[] __initconst = {
	{
		.matches = {
	...
	},
	{
		...
		.matches = {
			DMI_MATCH(DMI_BOARD_NAME, "LAPKC71"),
			{ }
		}
	},
	{
...
struct nuc_wmi_features_struct nuc_wmi_features;
...
static int __init check_features_ec(void)
{
	...
	if (err >= 0) {
		nuc_wmi_features.super_key_lock = !!(err & SUPPORT_1_SUPER_KEY_LOCK);
		nuc_wmi_features.lightbar       = !!(err & SUPPORT_1_LIGHTBAR);
		nuc_wmi_features.fan_boost      = !!(err & SUPPORT_1_FAN_BOOST);
	} else {
...
static int __init check_features_bios(void)
{
	...
	if (!dmi_check_system(nuc_wmi_dmi_table)) {
	...
	if (bios_version >= 114) {
		...
		if (strspn(s, " ") == s_len) {
			nuc_wmi_features.fn_lock           = true;
			nuc_wmi_features.batt_charge_limit = true;
			nuc_wmi_features.fan_extras        = true;
		} else if (s_len > 0) {
			/* TODO */
...
int __init nuc_wmi_check_features(void)
{
	...
	/*
	 * Intel NUC X15 (LAPKC71F) has a lightbar but the EC SUPPORT_1
	 * register does not advertise it. Force-enable based on DMI match.
	 */
	{
		static const struct dmi_system_id nuc_x15_lightbar[] = {
			{
				.matches = {
					DMI_MATCH(DMI_BOARD_NAME, "LAPKC71"),
					{ }
		...
		if (!nuc_wmi_features.lightbar && dmi_check_system(nuc_x15_lightbar)) {
			pr_info("NUC X15 detected: force-enabling lightbar support\n");
			nuc_wmi_features.lightbar = true;
		}
		if (dmi_check_system(nuc_x15_lightbar)) {
			nuc_wmi_features.batt_charge_limit = true;
			nuc_wmi_features.fn_lock = true;
			nuc_wmi_features.fan_extras = true;
		}

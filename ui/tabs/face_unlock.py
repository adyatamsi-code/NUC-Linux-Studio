...
class FaceUnlockTab(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        self._install_btn = tk.Button(setup_row1, text="📦 Install Howdy",
                                      font=("Arial", 10, "bold"), fg="white", bg=t["svc_btn_load"],
        ...
        self._pam_btn = tk.Button(setup_row2, text="🔗 Enable PAM Integration",
                                  font=("Arial", 10, "bold"), fg="white", bg=t["svc_btn_rebuild"],
                                  relief="flat", padx=16, pady=6, width=_SETUP_BTN_W,
        ...
        self._howdy_status = tk.Label(row1, text="Checking…", font=("Arial", 10, "bold"), fg="gray", bg=t["bg"])
        ...
        self._camera_status = tk.Label(row2, text="Checking…", font=("Arial", 10, "bold"), fg="gray", bg=t["bg"])
        ...
        self._camera_device = tk.Label(row3, text="—", font=("Arial", 10), fg="gray", bg=t["bg"])
        ...
        self._faces_status = tk.Label(row4, text="Checking…", font=("Arial", 10, "bold"), fg="gray", bg=t["bg"])
        ...
        self._pam_status = tk.Label(row5, text="Checking…", font=("Arial", 10, "bold"), fg="gray", bg=t["bg"])
        ...
        style = ttk.Style()
        style.configure("Treeview", background=t["bg_secondary"], foreground=t["fg"],
                         fieldbackground=t["bg_secondary"], font=("Liberation Sans Narrow", 9), rowheight=34)
        style.configure("Treeview.Heading", background=t["bg"], foreground=t["accent"],
                         font=("Liberation Sans Narrow", 12, "bold"), padding=(6, 4))
        style.map("Treeview", background=[("selected", t["accent_hover"])],
        ...
        self._enroll_btn = tk.Button(btn_row, text="➕ Add Face", font=("Arial", 10),
                                     fg="white", bg=t["svc_btn_load"], relief="flat", padx=12, pady=4,
        ...
        self._add_snapshot_btn = tk.Button(btn_row, text="📸 Add Snapshot", font=("Arial", 10),
                                           fg="white", bg=t["svc_btn_restart"], relief="flat", padx=12, pady=4,
        ...
        self._delete_btn = tk.Button(btn_row, text="🗑 Delete Selected", font=("Arial", 10),
                                     fg="white", bg=t["svc_btn_unload"], relief="flat", padx=12, pady=4,
        ...
        self._rename_btn = tk.Button(btn_row, text="✏️ Rename", font=("Arial", 10),
                                      fg="white", bg=t.get("svc_btn_secondary", "#5A5070"), relief="flat", padx=12, pady=4,
        ...
        tk.Button(cert_row, text="Apply", font=("Arial", 9),
                  fg="white", bg=t["svc_btn_load"], relief="flat", padx=8,
        ...
        self._test_btn = tk.Button(preview_btn_row, text="🧪 Test Recognition", font=("Arial", 10),
                                    fg="white", bg=t["svc_btn_rebuild"], relief="flat", padx=12, pady=4,
    ...
    def apply_theme(self):
        ...
        self._faces_tree.tag_configure("snapshot",
            ...
            font=("Segoe UI", 11))
        # Re-color buttons
        self._enroll_btn.configure(bg=t["svc_btn_load"])
        self._add_snapshot_btn.configure(bg=t["svc_btn_restart"])
        self._delete_btn.configure(bg=t["svc_btn_unload"])
        self._rename_btn.configure(bg=t.get("svc_btn_secondary", "#5A5070"))
        self._refresh_btn.configure(bg=t["svc_btn_restart"])
        self._preview_btn.configure(bg=t.get("svc_btn_secondary", "#5A5070"))
        self._rgb_preview_btn.configure(bg=t.get("svc_btn_secondary", "#5A5070"))
        self._test_btn.configure(bg=t["svc_btn_rebuild"])
        # Re-color tk.Label status widgets background on theme change
        for lbl in (self._howdy_status, self._camera_status, self._camera_device,
                    self._faces_status, self._pam_status):
            lbl.configure(bg=t["bg"])
        # Refresh status foreground colors for the new theme
    ...
    def _update_ui(self, howdy_installed, ir_camera, faces, pam_services, certainty, device_path):
        ...
        show_setup = False
        if howdy_installed:
            self._howdy_status.config(text="Installed ✓", fg=t["status_green"], foreground=t["status_green"])
            self._install_btn.config(state=tk.DISABLED, text="✓ Howdy Installed", bg="#555")
        else:
            self._howdy_status.config(text="Not Installed ✗", fg=t["status_red"], foreground=t["status_red"])
            self._install_btn.config(state=tk.NORMAL, text="📦 Install Howdy", bg=t["svc_btn_load"])
            show_setup = True

        if ir_camera:
            self._camera_status.config(text=f"Detected ✓ ({ir_camera['name']})", fg=t["status_green"], foreground=t["status_green"])
            self._camera_device.config(text=ir_camera['device'], fg=t["fg"])
        else:
            self._camera_status.config(text="Not Detected ✗", fg=t["status_red"], foreground=t["status_red"])
            self._camera_device.config(text="—", fg="gray")

        if pam_services:
            self._pam_status.config(text=f"Enabled ({', '.join(pam_services)})", fg=t["status_green"], foreground=t["status_green"])
            ...
            self._pam_status.config(text="N/A", fg="gray")
        ...
        if faces:
            self._faces_status.config(
                ...
                fg=t["status_green"], foreground=t["status_green"])
        else:
            self._faces_status.config(
                text="No faces enrolled" if howdy_installed else "N/A",
                fg=t["status_red"] if howdy_installed else "gray", foreground=t["status_red"] if howdy_installed else "gray")
    ...
    def _stop_preview(self):
        ...
        from ui import themes
        t = themes.get()
        self._preview_btn.config(text="📷 IR Camera", bg=t.get("svc_btn_secondary", "#5A5070"))
        self._rgb_preview_btn.config(text="📹 RGB Camera", bg=t.get("svc_btn_secondary", "#5A5070"))

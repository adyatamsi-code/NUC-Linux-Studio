...
# Map: (display name, systemd service name, oneshot)
# oneshot=True  → service runs once at boot (Type=oneshot); show "Re-apply" instead of Restart
# oneshot=False → persistent daemon; show Restart + Stop
DAEMONS = [
    ("Touchpad Daemon",      "touchpad-led.service",      False),
    ("Kbd Brightness Daemon","kbd-brightness.service",     False),
    ("Fan Curve Daemon",     "fan-curve.service",          False),
    ("Audio Reactive Daemon","kbd-audio.service",          False),
    ("Battery Limit (boot)", "nuc-battery-limit.service", True),
]
...
class TogglesTab(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        # Daemon rows
        for label, svc_name, oneshot in DAEMONS:
            row_idx += 1
            ...
            if oneshot:
                # Oneshot: Re-apply button only (runs once, no persistent state to stop)
                reapply_btn = tk.Button(svc_frame, text="Re-apply", font=("Arial", 10),
                                        fg="white", bg=t["svc_btn_restart"], relief="flat", padx=10, pady=3,
                                        command=lambda s=svc_name: self._restart_daemon(s))
                reapply_btn.grid(row=row_idx, column=3, columnspan=2, sticky="e", padx=(0, 0), pady=3)
                self._daemon_btns[svc_name] = (reapply_btn, None)
            else:
    ...
    def _refresh_touchpad_status(self):
        ...
        try:
            ...
            if tp_state is not None:
                ...
                if tp_state:
                    self.touchpad_status_label.config(text="🖱️ Enabled", foreground=t["status_green"])
                else:
                    self.touchpad_status_label.config(text="🖱️✗ Disabled", foreground=t["status_red"])
            else:
    ...
    def _update_mic_label(self, muted):
        ...
        if muted is None:
            ...
            self._mic_status_label.config(text="🎙️✗ Muted", foreground=t["status_red"])
        else:
    ...
    def _is_daemon_running(self, svc_name: str) -> bool:
        try:
            r = subprocess.run(["systemctl", "is-active", svc_name],
                              capture_output=True, text=True, timeout=3)
            # "active" = running daemon; "active (exited)" = successful oneshot
            return r.stdout.strip() in ("active",)
        except Exception:
            return False

    def _is_daemon_ok(self, svc_name: str) -> tuple:
        """Return (ok: bool, status_text: str) for any service type."""
        try:
            r = subprocess.run(["systemctl", "is-active", svc_name],
                              capture_output=True, text=True, timeout=3)
            ...
            if state == "active":
                ...
                # Check if it's a oneshot that exited successfully
                r2 = subprocess.run(
                    ["systemctl", "show", svc_name, "--property=ActiveState,SubState,Result"],
                    capture_output=True, text=True, timeout=3)
                props = dict(line.split("=", 1) for line in r2.stdout.strip().splitlines() if "=" in line)
                if props.get("ActiveState") == "active" and props.get("SubState") == "exited" and props.get("Result") == "success":
                    return True, "Applied ✓"
    ...
    def _refresh_services(self):
        ...
        # Daemons
        for _, svc_name, _ in DAEMONS:
            lbl = self._daemon_labels.get(svc_name)
            if lbl:
                ok, text = self._is_daemon_ok(svc_name)
                lbl.config(text=text, foreground=t["status_green"] if ok else t["status_red"])
...
"""
    ...
    def _restart_daemon(self, svc_name: str):
        # Oneshot services use stop+start instead of restart so the ExecStart runs again
        oneshot = any(s == svc_name and o for _, s, o in DAEMONS)
        if oneshot:
            cmd = f"systemctl stop {svc_name}; systemctl start {svc_name}"
            label = "Re-applying"
            done = f"Re-applied {svc_name}"
        else:
            cmd = f"systemctl restart {svc_name}"
            label = f"Restarting {svc_name}..."
            done = f"Restarted {svc_name}"
        self._run_privileged(cmd, label, done, callback=self._refresh_services)
